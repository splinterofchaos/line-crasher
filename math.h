...
inline glm::vec3 to_vec3(const glm::vec2& v) { return glm::vec3(v.x, v.y, 0); }

glm::vec2 radial_vec(float radians, float length = 1);
inline glm::vec3 radial_vec3(float radians, float length = 1) {
  return to_vec3(radial_vec(radians, length));
}
